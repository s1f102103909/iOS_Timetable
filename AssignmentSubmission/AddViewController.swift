...
class AddViewController: UIViewController {
    ...
    @IBAction func save(){
        let object = NCMBObject(className: "Task")
        object?.setObject(taskTextView.text, forKey: "task")
        object?.setObject(taskDatetime.text, forKey: "date")
        ...
        if timeInterval_1hour != 0.0{
           ...
           //notificationDayの月日時分の情報をInt型で取得
            ...
            content.title = taskTextView.text!
        ...
        object?.setObject(id_1hour, forKey: "id_1hour")
        object?.setObject(id_1day, forKey: "id_1day")
        object?.setObject(id_2day, forKey: "id_2day")
        object?.saveInBackground({ (error) in
            if error != nil {
                print(error!)
            }
            else {
                let alertController = UIAlertController(title: "登録完了", message: "課題の登録が完了しました。", preferredStyle: .alert)
    ...
    @IBAction func notificationOn2(){
       ...
       //notificationDayの月日時分の情報をInt型で取得
        ...
        //通知登録
        if timeInterval_1day != 0{
            setLocalNotification(id: id_1day, title: content.title, message: content.body, month: month, day: day, hour: hour, minute: minute, second: 00)
        }
    }
     */
    /*
    @IBAction func notificationOn3(){
        let date = Date()
        print(date,"現在時刻")
        let dateString = taskDatetime.text
        // String→Dateに変換
    ...
    @IBAction func segmentedControlChanged(_ sender: UISegmentedControl) {
           ...
           case 0:
               timeInterval1 = -60.0*60
           case 1:
               timeInterval1 = -60.0*60*24
           case 2:
               timeInterval1 = -60.0*60*48
           default:
               break
           }
        let dateString = taskDatetime.text
        let date1 = AddViewController.dateFromString(string: dateString!, format: "YYYY年MM月dd日HH時mm分")!
        let newDate = Date(timeInterval: timeInterval1, since: date1)
        let newDateString = AddViewController.stringFromDate(date: newDate, format: "YYYY年MM月dd日HH時mm分")
       }
     */
    ...
    @IBAction func checkBox1(_ sender:UIButton){
        if button1.backgroundColor == UIColor.white{
            ...
            button1.tintColor = UIColor.systemBlue
    ...
    @IBAction func checkBox2(_ sender:UIButton){
        if button2.backgroundColor == UIColor.white{
            ...
            button2.tintColor = UIColor.systemBlue
    ...
    @IBAction func checkBox3(_ sender:UIButton){
        if button3.backgroundColor == UIColor.white{
            ...
            button3.tintColor = UIColor.systemBlue
